...
class TickData:
    ...
    def tick_data(self):
        ...
        print('Tick end')
t_120=TickData("F", market[0], 120, 100)
t_120.tick_data()
#----------------------------------------------------------
...
while True:
    ...
    cci120_p20=ta.trend.cci(low=t120["Low"].astype(float), high=t120["High"].astype(float), close=t120["Close"].astype(float), window=20, constant=0.015).dropna(axis=False)
    cci120=ta.trend.cci(low=t120["Low"].astype(float), high=t120["High"].astype(float), close=t120["Close"].astype(float), window=9, constant=0.015).dropna(axis=False)
    #볼린저밴드 제작
    ma=[12, 20]
    mul=[0.2, 1.8]
    ...
    def sellcondition(cci120):
        ...
        for i in min_kind:
            ...
            #120틱 CCI 조건
            cciupover=cci120>=100
            cciup_over=any(cciupover[-3:]==True) and (cci120[-3]>=100 and cci120[-2]<100)
            #조건취합
    ...
    def buycondition(cci120):
        ...
        for i in min_kind:
            ...
            #120틱 CCI 조건
            ccidnover=cci120<=-100
            ccidn_over=any(ccidnover[-3:]==True) and (cci120[-3]<=-100 and cci120[-2]>-100)
            #조건취합
        ...
        buy=any(buyconlist)
        return buy    
    sellenter=sellcondition(cci120)
    buyenter=buycondition(cci120)
    #진입 조건 (프로그램 연계)
        #매수조건     
    if states==0:  
        if (buyenter==True):
            states=1 # **상태메세지를 매수일 때로 변경(상태메세지 : 1로 변경)              
            p_cur=float(t120['Close'][-1]) # 매수 시점의 가격을 언제든지 불러올 수 있도록 변수에 넣어서 저장 *상태메세지와 비교 대조하며 작동상황을 확인하기 위해 작성
            loss_open=float(t120['Open'][-1]) #손실방어를 위한 시가저장
            pyautogui.click(x=2916, y=166, duration=0.02) # 청산버튼 클릭            
            pyautogui.click(x=3061, y=165, duration=0.02) # 매수버튼 클릭        
            # 매수 상태 및 가격, 현재 시간 등을 어딘가에 기록(상태메세지 : Tick Danta 매수신호 발생. 매수진입)
            pyperclip.copy("Hangseng Tick Danta 매수신호 발생. 매수진입 \n\n" +
        ...
        #(매도조건)             
            ...
            p_cur=float(t120['Close'][-1]) # 매도 시점의 가격을 언제든지 불러올 수 있도록 변수에 넣어서 저장 *상태메세지와 비교 대조하며 작동상황을 확인하기 위해 작성
            loss_open=float(t120['Open'][-1]) #손실방어를 위한 시가저장
            pyautogui.click(x=2916, y=166, duration=0.02) # 청산버튼 클릭            
            pyautogui.click(x=2683, y=166, duration=0.02) # 매도버튼 클릭                  
            # 매도 상태 및 가격, 현재 시간 등을 어딘가에 기록(상태메세지 : Tick Danta 매도신호 발생. 매도진입)
            pyperclip.copy("Hangseng Tick Danta 매도신호 발생. 매도진입 \n\n" +
            '현재시간 : '+str(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+
            '\n'+'현재상태 : '+str(states)+
            '\n' +'현재가격 : '+t120['Close'][-1]+
            '\n'+'진입방향 : 하방')
    ...
    #진입상태일 때        
        if states == 1:
            ...
            #매도 스위칭. 역방향 진입자리 발생  
                ...
                pyautogui.click(x=2683, y=166, duration=0.02) # 매도버튼 클릭                  
                # 매도 상태 및 가격, 현재 시간 등을 어딘가에 기록(상태메세지 : Tick Danta 매도신호 발생. 매도진입)
                pyperclip.copy("Switching. Hangseng Tick Danta 매도신호 발생. 매도진입 \n\n" +
                '현재시간 : '+str(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+
                '\n'+'현재상태 : '+str(states)+
                '\n' +'현재가격 : '+t120['Close'][-1]+
                '\n'+'진입방향 : 하방')
                ...
                pyautogui.hotkey('enter') 
                time.sleep(0.3)                       
            elif ((cci120_p20[-3]>cci120_p20[-2]) and (cci120_p20[-3]>=100))==True: 
                # 매도 상태 및 가격, 현재 시간 등을 어딘가에 기록(상태메세지 : 반대매도신호 발생 올청산)
                profit_stack=profit_stack+1
                states=0  
                pyautogui.click(x=2916, y=166, duration=0.02) # 청산버튼 클릭                       
                pyperclip.copy("Hangseng 반대 매도신호 발생. Buy Position 익절 \n\n" + 
                '현재시간 : '+str(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+
                '\n'+'현재상태 : '+str(states)+
                '\n' +'현재가격 : '+t120['Close'][-1]+
                '\n'+'내용 : 익절청산')
            ...
            if (t120['High'].rolling(window=1).min()[-1] >= p_cur+32) ==True: 
                # **상태메세지에 기록(상태메세지를 0으로 변경)
                states=0  
                #손절이 나면 스택을 쌓는다.
                loss_stack=loss_stack+1                
                # 현재 상태 및 가격, 현재 시간 등을 어딘가에 기록(기록메세지 : Sell 진입 손절)
                pyperclip.copy("Hangseng Sell 진입 손절 \n\n" +
                '현재시간 : '+str(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+
                '\n'+'현재상태 : '+str(states)+
                '\n' +'현재가격 : '+t120['Close'][-1])
                pyautogui.click( x=2050, y=346)
            ...
            #매수 스위칭. 역방향 진입자리 발생  
                ...
                pyautogui.click(x=3061, y=165, duration=0.02) # 매수버튼 클릭        
                # 매수 상태 및 가격, 현재 시간 등을 어딘가에 기록(상태메세지 : Tick Danta 매수신호 발생. 매수진입)
                pyperclip.copy("Switching. Hangseng Tick Danta 매수신호 발생. 매수진입 \n\n" +
                '현재시간 : '+str(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+
                '\n'+'현재상태 : '+str(states)+
                '\n' +'현재가격 : '+t120['Close'][-1]+
                '\n'+'진입방향 : 상방')
                ...
                states=0 
                pyautogui.click(x=2916, y=166, duration=0.02) # 청산버튼 클릭             
                pyperclip.copy("Hangseng 반대 매수신호 발생. Sell Position 익절 \n\n" +
                '현재시간 : '+str(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+
                '\n'+'현재상태 : '+str(states)+
                '\n' +'현재가격 : '+t120['Close'][-1]+
                '\n'+'내용 : 익절청산')
    ...
    print("CCI :", cci120[-1]) 
    print("CCI period 20 :", cci120_p20[-1]) 
